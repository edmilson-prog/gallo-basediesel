/**
 * MIRROR of `src/features/quotes/engine/quoteMessage.ts`.
 *
 * The quote body has to be built in two runtimes: the browser (WhatsApp text,
 * preview) and Deno (the e-mail this function sends). Changed the app-side
 * engine? Copy it here and redeploy `quote-send-email`, or the customer starts
 * getting two different quotes depending on the channel.
 *
 * Kept dependency-free and DOM-free on purpose — same reason as
 * `_shared/fiscal/*`: Deno has no DOMParser.
 */

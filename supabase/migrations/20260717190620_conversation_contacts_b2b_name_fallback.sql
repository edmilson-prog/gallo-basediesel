-- B2B display-name fallback for the conversation identity RPCs.
--
-- Why: DINTEC-enriched customers can land as type='B2B' with an empty
-- nome_fantasia (the ERP only reliably fills the human/full name). The inbox
-- name resolves through these two SECURITY DEFINER RPCs, whose name expression
-- was `case when B2B then nome_fantasia else full_name end` — no fallback — so
-- an empty nome_fantasia produced a null name and the UI degraded to
-- "Lead anônimo" even though the ficha rendered correctly from the customer row.
--
-- What: only the display-name expression changes, to
--   case when B2B then coalesce(nullif(nome_fantasia,''), nullif(razao_social,''), full_name) else full_name end
-- The access gate (can_access_conversation), the query shape, filters, and every
-- other column are reproduced verbatim from the live definitions — this is a
-- pure presentation change, it does NOT touch the conversation access model.
--
-- The data was already backfilled (nome_fantasia := full_name for the 29
-- affected B2B rows, audit_logs customers_b2b_nome_fantasia_backfill); this
-- migration is the structural guard so a future incomplete B2B row can never
-- reintroduce the blank name. `idle_conversations_summary` is unchanged — it
-- already coalesces nome_fantasia -> full_name -> 'Contato'.

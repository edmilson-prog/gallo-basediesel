...
Estado atual: **Fase 2 (Mock → Supabase) — escopo A concluído e mergeado na `main`** (PR #39, codinome `Keystone`, v0.73.0). A Fase 1 (Frontend First) entregou o app completo sobre dados fictícios determinísticos (faker + seed) na arquitetura-alvo (`features/`, `mocks/`, `providers/`, `shared/`); a maioria dos PRDs comerciais está `_DONE` — incluindo gestão de mídia (PRD-026, "Vault") e envio rápido/biblioteca de ativos (PRD-027, "Dispatch"). O **backend do cutover está pronto e validado**: 34 providers Supabase, 39 tabelas (schema `public`, todas com RLS — write policies e isolamento per-seller incluídos), seed populado, **auth real switchável** (`VITE_AUTH_SOURCE=mock|supabase`) com Custom Access Token Hook habilitado, 5 Edge Functions de gestão de usuários e `pg_cron` recompondo notificações derivadas no servidor. Suíte de regressão de RLS versionada em `supabase/tests/rls-regression.sql` (workflow de CI no-op até existir o secret `SUPABASE_DB_URL`). A faixa de infra **PRD-100–108 foi fechada no PR #50** (v0.74.0 `Backbone`): as migrations agora estão **espelhadas em `supabase/migrations/`** (⚠️ regra: todo `apply_migration` via MCP deve ser exportado para o Git no mesmo PR), Edge Functions padronizadas em `_shared/`, Realtime core no Inbox, Storage com upload real e MVs de BI via RPCs scoped. **Produção segue em `mock` por default** (o Preview da Vercel roda `supabase`); o fechamento é gated no dono: **#45** (secrets de CI), **#46** (allowlist da redirect URL + teste e2e do convite — secrets Resend já setados), **#47** (flip de produção). A **loja transacional B2C** (#40/#41) foi deferida para fase própria — em `supabase` o checkout faz handoff por WhatsApp (write-free). Doc mestre de pendências: `docs/fase2-pendencias.md`. Projeto mantido pela AILA Sistemas Inteligentes.
...
- Faixa **100+** (infra Supabase / Fase 2): **`PRD-100` a `PRD-108` estão `_DONE`** (2026-06-09), cada um com ressalvas anotadas no cabeçalho do próprio arquivo (CI gated em secrets #45, Resend #46, itens deferidos com motivo). Destaques entregues: migrations versionadas em `supabase/migrations/` (espelho do remoto — manter sincronizado a cada `apply_migration`), `_shared/` + canário nas Edge Functions, Realtime core no Inbox (gate por `VITE_DATA_SOURCE`), Storage com upload real de mídia, MVs de BI com refresh `pg_cron` lidas via RPCs scoped. Seguem **pendentes**: `PRD-109` (backup/DR) e `PRD-110` (monitoring). `PRD-201` e os épicos `PRD-111`–`PRD-120` (WhatsApp real) e `PRD-124`–`PRD-129` (DINTEC/NF-e) são backlog futuro, não iniciados.
...
- **SemVer.** MINOR/MAJOR recebem **codinome em inglês** (atual: `Backbone` — v0.74.0).
...
- **Tags git** acompanham cada bump: `vX.Y.Z` no commit de release/merge (ex.: `v0.67.0` Vault, `v0.68.0` Dispatch, `v0.69.0` Beacon, `v0.70.0` Guide, `v0.71.0` Focus, `v0.72.0` Bedrock, `v0.73.0` Keystone, `v0.74.0` Backbone).

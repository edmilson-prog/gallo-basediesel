...
- **SemVer.** MINOR/MAJOR recebem **codinome em inglês** (atual: `Tally` — v0.181.0). Cada bump ganha uma tag `vX.Y.Z` (lista completa: `git tag -l`).
...
- 🔄 **Todo bump sincroniza a `main` local.** Assim que a versão entra na `main` remota (merge do PR de bump + tag), rode no **diretório principal** — nunca numa worktree:

  ```bash
  git -C D:/claude/gallo-basediesel fetch origin && git -C D:/claude/gallo-basediesel merge --ff-only origin/main
  ```

  Merge feito pela UI do GitHub **não** atualiza clone nenhum, e worktree nova nasce da `origin/main` **em cache** — uma `main` local velha gera worktree velha, com `CLAUDE.md`, `CHANGELOG.md`, migrations e contagem de testes divergentes do que está em produção. Sempre `--ff-only`: o principal só avança, nunca ganha merge commit. Ao terminar, o principal deve fechar em **zero commits** de diferença para `origin/main`.

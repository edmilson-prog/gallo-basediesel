...
- Faixa **100+** (infra Supabase / Fase 2): `PRD-103` (RLS) e `PRD-104` (providers) estão `_DONE` com ressalvas anotadas no cabeçalho de cada arquivo; `PRD-100`/`PRD-101`/`PRD-107` estão quase completos (faltam CI de migrations/tipos, runbooks e a rota `/auth/definir-senha`); `PRD-102` (scaffold de Edge Functions), `PRD-105` (realtime — hoje só simulador mock), `PRD-106` (storage — 0 buckets criados), `PRD-108` (perf — só índices + initplan), `PRD-109` (backup/DR) e `PRD-110` (monitoring) seguem **pendentes**. `PRD-201` e os épicos `PRD-111`–`PRD-120` (WhatsApp real) e `PRD-124`–`PRD-129` (DINTEC/NF-e) são backlog futuro, não iniciados.

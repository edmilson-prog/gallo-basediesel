...
- Faixa **100+** (infra Supabase / Fase 2): **`PRD-100` a `PRD-110` estão `_DONE`** (100–108 em 2026-06-09; 109/110 em 2026-06-10, PR #51), cada um com ressalvas anotadas no cabeçalho do próprio arquivo (CI gated em secrets #45, ativação DR/observabilidade #52, itens deferidos com motivo). Destaques entregues: migrations versionadas em `supabase/migrations/` (espelho do remoto — manter sincronizado a cada `apply_migration`), `_shared/` + canário nas Edge Functions, Realtime core no Inbox (gate por `VITE_DATA_SOURCE`), Storage com upload real de mídia, MVs de BI com refresh `pg_cron` lidas via RPCs scoped, workflows de backup + runbooks de DR (`docs/infra/`), Sentry gated, healthcheck público e dashboard de saúde Owner-only (`docs/ops/observability.md`). `PRD-201` e os épicos `PRD-111`–`PRD-120` (WhatsApp real) e `PRD-124`–`PRD-129` (DINTEC/NF-e) são backlog futuro, não iniciados.
...
- **SemVer.** MINOR/MAJOR recebem **codinome em inglês** (atual: `Sentinel` — v0.75.0).
...
- **Tags git** acompanham cada bump: `vX.Y.Z` no commit de release/merge (ex.: `v0.67.0` Vault, `v0.68.0` Dispatch, `v0.69.0` Beacon, `v0.70.0` Guide, `v0.71.0` Focus, `v0.72.0` Bedrock, `v0.73.0` Keystone, `v0.74.0` Backbone, `v0.75.0` Sentinel).

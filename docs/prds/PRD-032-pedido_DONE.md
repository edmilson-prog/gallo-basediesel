...
| **Status** | ✅ IMPLEMENTADO                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **Data**   | 2026-05-26                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **Notas**  | Tipos `IOrder`/`IOrderItem` estendidos (preservando enums em PT existentes em mocks e ficha do cliente). Status agregado `OrderStatus` derivado via `computeOrderStatus`. Geração de mocks atualizada para a distribuição alvo (30% aguardando, 25% concluído, 15% enviado, etc.) com `number`, `paidAt`, `shippedAt`, endereço de entrega, `commissionPreview` e `conversationId` em ~25% dos pedidos. Listagem em `/app/pedidos` com filtros agregados + pagamento + entrega + origem + URL sync. Detalhe `/app/pedidos/:id` com 7 seções, transições contextuais (pago → separação → enviado → entregue), aplicação de item a veículo gerando `IVehicleServiceEntry`, cancelamento com regras por estado, devolução, refund e NF placeholders. `createOrderFromQuote` substitui o stub inline do `QuoteDetailPage` e do `useSdrResponder` (aceite SDR cria pedido real). Tab Pedidos da ficha (`OrdersTab`) agora delega ao `CustomerOrdersList`. |

...
### Onda 6 — DINTEC + NFe Própria (PRDs 121–131)

> **⚠️ Risco DINTEC sem API materializado (27/05/2026).** Onda re-estruturada: leitura direta no banco DINTEC + watcher de export CSV + emissor NFe próprio (NFE.io / eNotas / PlugNotas).
> Versão alvo: **v2.2.0 — codinome Sync**

| #    | Título                                                           | Tipo       | Prof. | Prior. | Documento | Status | Depende de    |
| ---- | ---------------------------------------------------------------- | ---------- | ----- | ------ | --------- | ------ | ------------- |
| 121  | Discovery DINTEC (sem API — banco, exports, workflow fiscal)     | Integração | D     | P0     | ⏸         | ⏳     | —             |
| 122  | Provider DINTEC base (DB connector read-only + file watcher)     | Integração | D     | P0     | ⏸         | ⏳     | 121, 102      |
| 123  | Sync de Catálogo (leitura direta no banco DINTEC)                | Feature    | D     | P0     | ⏸         | ⏳     | 122, F1:030   |
| 124  | Sync de Estoque (view SQL near real-time)                        | Feature    | D     | P0     | ⏸         | ⏳     | 122, F1:030   |
| 125  | Sync de Clientes (read DINTEC + escrita só GALLO — ilha parcial) | Feature    | D     | P1     | ⏸         | ⏳     | 122, F1:012   |
| 126  | Export de Pedidos (CSV/XML para DINTEC consumir)                 | Feature    | D     | P0     | ⏸         | ⏳     | 122, F1:032   |
| 127  | Provider NFe Próprio (abstração NFE.io / eNotas / PlugNotas)     | Integração | D     | P0     | ⏸         | ⏳     | 102, 104      |
| 127B | Emissão NFe via Provider (substitui placeholder PRD-032)         | Feature    | D     | P0     | ⏸         | ⏳     | 127, 126, 106 |
| 128  | Conflict Resolution Cross-System                                 | Feature    | D     | P1     | ⏸         | ⏳     | 123–127B      |
| 129  | Importação CSV Robusta (promovido a P0 — primário)               | Feature    | D     | P0     | ⏸         | ⏳     | 123           |
...
| **Motivo**           | Atualização v1.1 — decisões tomadas (Asaas+Mercado Pago, Resend, 3 LLMs); DINTEC sem API materializado (Onda 6 re-estruturada); +5 PRDs inseridos (127B, 131B, 151B, 151C, 151D) com inspiração nos prints do projeto Dermatobel. Total: **105 PRDs**. |

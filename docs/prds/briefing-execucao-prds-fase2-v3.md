...
A Fase 2 está organizada em **10 ondas** totalizando **105 PRDs** na faixa 100–200 (4 PRDs inseridos via sufixo `B/C/D` em Ondas 6, 7 e 9 após decisões de 27/05/2026). A **Onda 14 (Fase 3)** introduz a faixa 201+ para "Operações Críticas" (PRD-201 já redigido):
...
| 6      | 121–131     | 11             | **DINTEC via CSV + NFe Própria** (sem API/banco)       | **14–18 semanas** | P0              |
...
| **14** | **201–210** | **1+10 slots** | **Operações Críticas (Fase 3 — pós-go-live)**          | **8–10 semanas**  | **P1**          |
...
| `IStockPolicy`            | 14   | PRD-201       | Política de estoque crítico (3 níveis: global/marca/SKU)     |
| `ISupplier`               | 14   | PRD-201       | Cadastro de fornecedor por marca (canais de notificação)     |
| `ISkuStockStatus`         | 14   | PRD-201       | Cache da zona atual do SKU (verde/amarela/vermelha)          |
| `IStockOverrideRequest`   | 14   | PRD-201       | Solicitação de exceção (vendedor pede, gestor aprova)        |
| `IStockOverrideLog`       | 14   | PRD-201       | Log imutável de decisões de override                         |
| `IStockPolicyChangeLog`   | 14   | PRD-201       | Log imutável de mudanças de política                         |
| `IStockNotificationLog`   | 14   | PRD-201       | Log de notificações disparadas (gestor + fornecedor)         |
...
> **⚠️ Atualização crítica (27/05/2026):** DINTEC **não expõe API nem permite acesso ao banco**. Cliente confirmou que limitação é institucional do fornecedor DINTEC, sem caminho de negociação técnica. Onda 6 re-escopada para **integração unidirecional via CSV manual** (operador exporta do DINTEC, faz upload no GALLO). Saldo no GALLO passa a ser **snapshot do último import** (defasagem aceita até 24h). Substituição do DINTEC fica registrada como **norte estratégico das Fases 4–5** (§14).

| #    | Título                                                                       | Tipo       | Prof. | Prior. | Complexidade | Depende de       |
| ---- | ---------------------------------------------------------------------------- | ---------- | ----- | ------ | ------------ | ---------------- |
| 121  | Discovery do workflow fiscal DINTEC + formatos de export                     | Integração | D     | P0     | Média        | —                |
| 122  | Interface de upload CSV/XML genérica (drag-drop + validação)                 | Feature    | D     | P0     | Média        | 102, 104         |
| 123  | Import de Catálogo via CSV (mapeamento de colunas + dry-run)                 | Feature    | D     | P0     | Alta         | 122, Fase 1: 030 |
| 124  | Import de Estoque via CSV (snapshot diário + diff)                           | Feature    | D     | P0     | Alta         | 122, Fase 1: 030 |
| 125  | Cadastro de Clientes no GALLO (fonte própria, sem dependência DINTEC)        | Feature    | D     | P0     | Média        | Fase 1: 012      |
| 126  | Export de Pedidos GALLO → CSV/XML para DINTEC consumir                       | Feature    | D     | P0     | Alta         | Fase 1: 032      |
...
| 128  | Reconciliação Manual GALLO ↔ DINTEC (relatório de divergências)              | Feature    | D     | P1     | Média        | 123, 124, 126    |
| 129  | Scheduler de Import + UI de Status (frequência configurável, default diário) | Feature    | D     | P0     | Média        | 123, 124         |
| 130  | Audit Cross-System (rastreamento de cada linha importada/exportada)          | Feature    | D     | P1     | Média        | 123–128          |

**Reflexos da decisão DINTEC sem acesso:**

- Onda 6 **não tem mais "Provider DINTEC"** — não há o que conectar
- Saldo de estoque no GALLO é **snapshot** (atualizado via import periódico, default diário). Toda tela crítica exibe timestamp do último import e aviso de defasagem.
- **GALLO vira fonte primária dos clientes** desde a Fase 2 (PRD-125). DINTEC continua tendo cópia via export manual.
- **NFe via emissor próprio** (PRDs 127/127B) — não há fallback possível pelo DINTEC
- Onda 6 **encolhe em complexidade** (sem provider integrado) mas **mantém duração de 14–18 semanas** devido à robustez exigida do scheduler/diff/reconciliação
...
> **Nota:** Onda primeira da **Fase 3** (pós-go-live Fase 2). Endereça itens stop-the-line cuja ruptura tem impacto operacional crítico no cliente final (transportadoras com caminhões parados). PRD-201 já redigido em 27/05/2026 (renumerado de PRD-150).

| #       | Título                                                                             | Tipo    | Prof. | Prior. | Complexidade | Depende de                                                          |
| ------- | ---------------------------------------------------------------------------------- | ------- | ----- | ------ | ------------ | ------------------------------------------------------------------- |
| 201     | Gestão Crítica de Estoque (Filtros — stop-the-line)                                | Feature | D     | P1     | Alta         | 122, 129, 114, 115, 141, 143, F1: 006, 019, 030, 031, 032, 052, 064 |
...
| Visão                                  | Quantidade                      |
| -------------------------------------- | ------------------------------- |
| **Total PRDs Fase 2 (Ondas 4–13)**     | 105                             |
| **PRDs Onda 14 (Fase 3 — registrado)** | 1 redigido + 9 slots reservados |
| **Total redigido neste documento**     | 106                             |
| **Detalhados (D)**                     | 80                              |
| **Esqueletos (E)**                     | 26                              |
| **Integrações (I/Integração)**         | 21                              |
| **P0 (bloqueante go-live Fase 2)**     | 31                              |
| **P1 (alta)**                          | 31 (inclui PRD-201)             |
| **P2 (média)**                         | 21                              |
| **P3 (futuro)**                        | 23                              |
...
## 14. Visão de Longo Prazo — Fases 3 a 5

> **Documento estratégico, não vinculante.** Esta seção registra a **direção** de evolução do projeto além da Fase 2. Não detalha PRDs; serve como **norte arquitetural** para decisões de modelagem na Fase 2 (ex: quais entidades devem ser primárias no GALLO antecipando substituição do DINTEC).

### 14.1 Por que existem Fases 3–5

A Fase 2 entrega backend real, integrações externas (WhatsApp, pagamentos, NFe própria, LLM, notificações) e elimina o "modo demonstração". Após go-live e validação operacional, três motivadores empurram o projeto para fases subsequentes:

1. **DINTEC limita acesso institucionalmente** — confirmado em 27/05/2026: não há API, nem acesso a banco, nem caminho de negociação técnica. Integração via CSV manual é solução paliativa, não permanente. Substituir DINTEC vira pauta inevitável.
2. **Operações críticas pedem mais do que vigilância de estoque** — PRD-201 (filtros) abre família de PRDs irmãos: outros itens stop-the-line, portal próprio do fornecedor, inteligência de compra com IA.
3. **GALLO como fonte da verdade comercial** — com clientes próprios (PRD-125), pedidos próprios e NFe própria, o GALLO já fica posicionado para assumir mais responsabilidades do ERP.

### 14.2 Fase 3 — Operações Críticas Expandidas

**Foco:** vigilância operacional avançada e fornecedores integrados.
**Início estimado:** pós-go-live Fase 2 (Mês 12+).
**Faixa de PRDs:** 201–250.

| Onda | PRDs    | Tema                                                                               | Status           |
| ---- | ------- | ---------------------------------------------------------------------------------- | ---------------- |
| 14   | 201–210 | Operações Críticas (estoque stop-the-line)                                         | PRD-201 redigido |
| 15   | 211–220 | Portal Próprio do Fornecedor (fornecedor entra, confirma prazos, registra pedidos) | a desenhar       |
| 16   | 221–230 | Inteligência de Compra (IA sugere o que comprar, quando, de quem)                  | a desenhar       |
| 17   | 231–240 | Logística Avançada (roteirização, frota própria/terceirizada)                      | a desenhar       |
| 18   | 241–250 | Gestão de Garantias e Devoluções Avançada                                          | a desenhar       |

### 14.3 Fase 4 — GALLO ERP Light (Substituição Parcial do DINTEC)

**Foco:** GALLO assume catálogo, estoque e clientes como **fonte primária**; DINTEC vira leitura/espelho ou é descontinuado nessas áreas.
**Início estimado:** +12–18 meses após go-live Fase 2.
**Faixa de PRDs:** 251–300.

| Onda | PRDs    | Tema                                                                             |
| ---- | ------- | -------------------------------------------------------------------------------- |
| 19   | 251–260 | GALLO Catálogo Primário (cadastro, fotos, OEM, compatibilidades, importação BPM) |
| 20   | 261–270 | GALLO Estoque Primário (entrada, saída, ajuste, contagem, inventário, perdas)    |
| 21   | 271–280 | GALLO Clientes Primário (já iniciado na Fase 2 PRD-125, aqui é endurecido)       |
| 22   | 281–290 | Compras Próprias (cotação, pedido de compra, recebimento, conferência)           |
| 23   | 291–300 | Migração Assistida (cliente faz cutover gradual, com janelas de validação)       |

### 14.4 Fase 5 — GALLO ERP Completo (Substituição Total do DINTEC)

**Foco:** financeiro, fiscal e contábil próprios. DINTEC é desligado.
**Início estimado:** +24–36 meses após go-live Fase 2.
**Faixa de PRDs:** 301–400.

| Onda | PRDs    | Tema                                                                      |
| ---- | ------- | ------------------------------------------------------------------------- |
| 24   | 301–315 | Financeiro (contas a pagar/receber, fluxo de caixa, conciliação bancária) |
| 25   | 316–330 | Fiscal Avançado (regimes tributários, SPED, EFD, integração contador)     |
| 26   | 331–345 | Folha + Comissões em Produção (substitui placeholder atual)               |
| 27   | 346–360 | Reports Contábeis (Balancete, DRE oficial, Razão, Diário)                 |
| 28   | 361–380 | DINTEC Desligado (cutover final, arquivamento histórico, decommissioning) |
| 29   | 381–400 | GALLO White-label (outras distribuidoras podem licenciar o sistema)       |

### 14.5 Impactos da visão de longo prazo nas decisões da Fase 2

A consciência da substituição futura do DINTEC justifica decisões mais ambiciosas na Fase 2 que de outra forma pareceriam exageradas:

| Decisão Fase 2                                  | Por que faz sentido com a visão de longo prazo             |
| ----------------------------------------------- | ---------------------------------------------------------- |
| PRD-125 — GALLO vira fonte primária de clientes | Base para Fase 4 Onda 21 (clientes primários endurecidos)  |
| PRD-127/127B — Emissor NFe próprio              | Indispensável quando DINTEC sair (Fase 5)                  |
| PRD-101 — Schema rico do banco Supabase         | Tabelas modeladas para acomodar evolução para ERP completo |
| PRD-110 — Observabilidade desde dia 1           | Critical quando GALLO virar único sistema operacional      |
| PRD-103 — RLS rigorosa                          | Multi-tenant pronto para White-label (Fase 5 Onda 29)      |
| PRD-191 — LGPD avançado                         | Pré-requisito para qualquer ERP em produção                |

### 14.6 Quando este documento será atualizado

Esta seção é viva. Cada conclusão de onda da Fase 2 motivará revisão:

- **Pós-Onda 6:** validar se import CSV diário do DINTEC é sustentável ou se Fase 4 precisa antecipar
- **Pós-Onda 10:** com B2B ativo, validar se Portal Fornecedor (Onda 15) sobe de prioridade
- **Pós-go-live Fase 2 (~mês 12):** sessão dedicada para detalhar PRDs da Fase 3

---

## 15. Apêndices

### 15.1 Glossário Fase 2
...
### 15.2 Decisões tomadas (27/05/2026)
...
### 15.4 Identidade preservada da Fase 1
...
### 15.5 Princípios consolidados da Fase 1 que continuam válidos
...
| 1.2    | 27/05/2026 | DINTEC sem qualquer acesso (nem banco) confirmado pelo cliente. Onda 6 re-escopada para **import CSV manual + GALLO como fonte primária de clientes**. PRD-150 renumerado para **PRD-201** e movido para **Onda 14 — Operações Críticas (Fase 3)**. Adicionado §14 — Visão de Longo Prazo (Fases 3, 4, 5) com substituição gradual do DINTEC. Total: **106 PRDs redigidos** (105 Fase 2 + 1 Onda 14). |

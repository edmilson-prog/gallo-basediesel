...
> Junção seller↔ativo (favoritos da biblioteca). `🔍 inferido (nome + CLAUDE.md/PRD)`
...
|--:|--------|------|:----:|---------|------------|
| 1 | `seller_id` | uuid | não | — | **PK** · FK → `sellers.id` |
| 2 | `asset_id` | uuid | não | — | **PK** · FK → `asset_library_items.id` ‹on delete cascade› |
| 3 | `created_at` | timestamptz | não | `now()` | — |
...
### `asset_favorites_delete` — DELETE · roles: `{authenticated}`
...
### `asset_favorites_insert` — INSERT · roles: `{authenticated}`
...
### `asset_favorites_select` — SELECT · roles: `{authenticated}`
- **USING:** `((seller_id IN ( SELECT sellers.id FROM sellers WHERE (sellers.store_id = ( SELECT current_store_id() AS current_store_id)))) AND (( SELECT is_staff() AS is_staff) OR (seller_id = ( SELECT current_seller_id() AS current_seller_id))))`

### `asset_favorites_update` — UPDATE · roles: `{authenticated}`
- **USING:** `((seller_id IN ( SELECT sellers.id FROM sellers WHERE (sellers.store_id = ( SELECT current_store_id() AS current_store_id)))) AND (( SELECT is_staff() AS is_staff) OR (seller_id = ( SELECT current_seller_id() AS current_seller_id))))`
- **WITH CHECK:** `((seller_id IN ( SELECT sellers.id FROM sellers WHERE (sellers.store_id = ( SELECT current_store_id() AS current_store_id)))) AND (( SELECT is_staff() AS is_staff) OR (seller_id = ( SELECT current_seller_id() AS current_seller_id))))`
...
- `asset_favorites_pkey` — `CREATE UNIQUE INDEX asset_favorites_pkey ON public.asset_favorites USING btree (seller_id, asset_id)`
- `asset_favorites_seller_id_idx` — `CREATE INDEX asset_favorites_seller_id_idx ON public.asset_favorites USING btree (seller_id)`
- `idx_asset_favorites_asset_id` — `CREATE INDEX idx_asset_favorites_asset_id ON public.asset_favorites USING btree (asset_id)`

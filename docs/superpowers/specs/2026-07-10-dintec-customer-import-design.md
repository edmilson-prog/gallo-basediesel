...
|---|---|
| `nome_fantasia` / `full_name` | Cadeia de resolução (decisão do dono, 2026-07-11): `NOTAFISCAL.NOME` → `NFISCAL.NOMECLI` → **`ORCAMENTO.NOME`** → `FANTASIA` → **`CONTATO`** → fallback sintético **`"Cliente DINTEC <CODCLI>"` + tag `Sem nome`**. `ORCAMENTO.NOME` entrou após o piloto (caso real CODCLI 2435 — só tinha orçamentos, nunca nota; backfillado em prod). `CONTATO` cobre 34 dos 574 sem nome da base completa. O fallback sintético mantém o cliente pesquisável e a tag `Sem nome` permite filtrar/renomear conforme forem identificados (aplicado retroativamente aos 9 do piloto). |
...
| Sem nome em **nenhuma** fonte (`NOTAFISCAL`+`NFISCAL`+`ORCAMENTO`+`FANTASIA`+`CONTATO`) | **574 na base completa** (varredura 2026-07-11); 9 no piloto (CODCLIs 4, 10, 11, 16, 31, 35, 36, 133, 165) | Cadastros-esqueleto do ERP (a `CLIENTE` do DINTEC **não tem** coluna de razão social — o nome vive nas notas/orçamentos). Composição dos 574: 571 têm documento, 378 têm telefone, 234 têm veículo, 34 têm `CONTATO` (vira o nome). Regra (decisão do dono): nome sintético `"Cliente DINTEC <CODCLI>"` + tag `Sem nome` (pesquisável, filtrável, renomeável) — aplicada retroativamente aos 9 do piloto. |
| Sem nome **e** sem documento **e** sem telefone **e** sem veículo | 3 na base completa | **Excluídos da importação** (peso morto absoluto — mesmo tratamento do CODCLI=1). Identificar os 3 CODCLIs na execução da Fase 3 e registrar no relatório. |

...
5. **Hard reload** — `hardReload` limpa o Cache Storage (remove chunks órfãos) e recarrega.
...
- **Service worker é seguro.** Existe `public/sw.js` (PRD-070) cache-first **só de assets** (style/script/image/font); ele **nunca** intercepta navegação/HTML (`request.mode === "navigate"` → rede). Logo `location.reload()` sempre pega um shell fresco, e `hardReload` limpando o Cache Storage remove os chunks órfãos que o SW tivesse guardado.
...
- `lib/buildId.test.ts` — 4 casos de `parseVersionJson` (JSON válido/ inválido/ sem `buildId`/ `buildId` vazio).

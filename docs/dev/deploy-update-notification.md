...
- **404 real para estático ausente (issue #430, pós-incidente de 17/08).** O rewrite SPA do `vercel.json` usa exclusão negativa (`/assets/`, `logos/`, `social/`, `version.json`, `CHANGELOG.md`, webmanifests, `sw.js`, favicons/ícones): arquivo estático **ausente** responde `404` de verdade em vez de `200 text/html`. Antes, o HTML-sob-URL-de-asset saía cacheável (`max-age=14400` na borda) e envenenava o HTTP cache do navegador na janela de troca de deploy — com 404, o auto-reload da rede de segurança **cura** em segundos em vez de prender. A matriz de paths é guardada por `engine/spaRewrite.test.ts`; `readChangelogPayload`/`classifyVersionResponse` completam o cinto e suspensório nos dois `fetch()` de dados que engoliam o HTML em silêncio. ⚠️ **Não adicionar `Cache-Control` longo/`immutable` para `/assets/` — medido e reprovado** (PR #516, fechado). Num deploy de preview, a Vercel anexou `max-age=31536000, immutable` **também à resposta 404** do mesmo path (headers do `vercel.json` casam por path, sem condicional de status). O caso perigoso não é aba velha pedindo chunk morto (inócuo: o build novo pede hashes novos) — é o de 17/08: aba **já no build atual** pede um chunk que **existe** mas ainda não propagou à borda; esse 404 ficaria cacheado por **1 ano**, matando a rota para aquele usuário com o arquivo intacto na origem, sem caminho de reload que alcance o HTTP cache. O destravamento é eliminar a janela de skew (Vercel *Skew Protection*), não cachear mais forte. Guardado por `engine/spaRewrite.test.ts`.

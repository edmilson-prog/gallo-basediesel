...
7. **Chamada ao adaptador** com `AbortSignal.timeout(LLM_TIMEOUT_MS ≈ 60 s)`:
   ...
   - **OpenAI** (`POST https://api.openai.com/v1/chat/completions`, `Authorization: Bearer`): a OpenAI **não** reporta custo monetário (só tokens) → custo = `usage.prompt_tokens/completion_tokens` × preço da linha persistida × `budget.usdToBrl` (mesmo caminho do Anthropic).

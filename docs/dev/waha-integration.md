...
- **`proxy`** (`{server, username?, password?}`) mapeia 1:1 para `config.proxy` (só incluído quando `server` está preenchido; limpar o campo servidor na UI descarta o objeto `proxy` inteiro, mantendo `provider_config` limpo).

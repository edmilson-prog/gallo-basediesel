...
Quando alguém responde uma conversa **direto pelo celular pareado** (fora da plataforma), a WAHA entrega o mesmo evento `message` com `fromMe: true` — até 2026-07-12 esse evento era reconhecido (`parser.ts` retorna `IOutboundEcho`) mas descartado (`markProcessed()` + `ignored: "outbound-echo"`), deixando um buraco invisível no histórico do Atendimento. Passou a ser espelhado, seguindo o mesmo desenho já validado em produção pela pipeline compartilhada (`_shared/whatsapp/webhook/core.ts`, seção "3.5 Outbound echoes" — **não importada** aqui, só o padrão foi replicado localmente por isolamento):

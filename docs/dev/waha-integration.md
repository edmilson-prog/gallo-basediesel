...
### Adoção do número canônico no match tolerante (self-heal, 2026-07-17)

O `findCustomerByPhone` do `waha-webhook` casa cliente por telefone com tolerância (sufixo de 8 dígitos + `phoneDigitsMatchBr` — DDI 55 opcional e 9º dígito para os dois lados). Quando o match é **tolerante** (dígitos armazenados ≠ dígitos do fio), o cliente agora **adota o número do fio** (`phone = '+' + dígitos`): o número que chega no evento é a identidade canônica do próprio WhatsApp (pn normalizado pelo usync), então essa é exatamente a confirmação que a regra do PR #302 exige para mexer em 9º dígito. Isso impede a base de regredir para os desvios corrigidos em lote em 2026-07-17 (DDI ausente / 9º dígito invertido, que quebravam o envio com GOWS 500 sem corpo).

Guard-rails: a adoção **nunca** acontece quando o número veio do fallback de `@lid` não resolvido (`lidUnresolved`/`toLidUnresolved` — placeholder não é telefone); é best-effort (falha vira `warn` no log, jamais bloqueia a recepção); e só escreve quando os dígitos realmente diferem. Cada adoção sai como log estruturado `"customer adopted canonical phone"` com `from`/`to` — pesquisável nos logs da função.

### Resolução de Lead para número desconhecido (Funnel Frente 3)

Quando `findCustomerByPhone` não acha ninguém (e o número **não** veio do fallback de `@lid` não resolvido — esse caso continua criando a âncora `pending_review` mínima, inalterado), o `waha-webhook` passa a resolver/criar um **Lead** em vez de um `customer` fantasma: `findLeadByPhone` (mesmo padrão tolerante do `findCustomerByPhone`) casa um lead existente (reabrindo se estava perdido, atribuindo dono via rodízio se estava sem dono e a mensagem é inbound vivo) ou cria um lead novo com dono via `assign_next_from_rotation`. O eco de saída faz o mesmo casamento/criação, mas nunca atribui rodízio. Documentação completa (modelo, régua de vitalidade de 7 dias, migração do acervo, imports, rollout): `docs/dev/funnel-frente3.md`.
...
- **Webhook**: eventos assinados via `X-Webhook-Hmac` (HMAC-SHA512 sobre o corpo bruto da requisição, chave = segredo do Vault apontado por `waha_servers.webhook_hmac_ref`), verificação em tempo constante (`timingSafeEqualStrings`). Lista de eventos subscritos na criação/atualização da sessão: `WAHA_DEFAULT_EVENTS = ["message", "message.any", "session.status", "message.ack", "message.reaction"]`. `message` cobre só mensagens recebidas de terceiros; `message.any` é o único canal por onde a WAHA entrega `fromMe: true` (eco de saída, seção "Espelhamento de eco" acima) — os dois nunca se sobrepõem no mesmo envelope, então não há double-processing por assinar ambos. `message.ack` (adicionado 2026-07-15) reporta transições de status de entrega/leitura (`ERROR:-1, PENDING:0, SERVER:1, DEVICE:2, READ:3, PLAYED:4` — mapeado por `mapWahaAckToStatus` em `ack.ts`) para mensagens que NÓS enviamos; o handler busca a mensagem por `provider_message_id` e aplica via `statusAdvances` (mesmo ranking anti-regressão que o `whatsapp-webhook` de referência usa, reaproveitado sem importar o core congelado). `message.reaction` (adicionado 2026-07-21) reporta 👍/❤️/etc. anexados a uma mensagem já persistida (ou `""` para remover) — patcheia `messages.reactions` sem tocar a conversa, exceto numa reação genuína do cliente, que bumpa `last_message_at`/`unread_count` (detalhes: `docs/dev/waha-payment-and-reactions.md`). Sessões criadas ANTES de cada uma dessas mudanças precisam de `updateConfig` (`waha-connect`) ou do script de re-inscrição correspondente (`scripts/waha-resubscribe-message-ack.ts`, `scripts/waha-resubscribe-reactions.ts`) para passar a receber o evento novo. A checagem "esse número tem WhatsApp?" (`whatsapp-check-number`) também passou a suportar contas WAHA via `GET /api/contacts/check-exists?phone=&session=` → `{numberExists, chatId}`.

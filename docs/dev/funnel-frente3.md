...
**Decisão registrada — RPCs órfãs e `HIDDEN_CUSTOMER_TAGS` ficam**: as RPCs `convert_pending_contact`, `mark_contact_not_customer` e `restore_pending_contact` (era `customers pending_review` pré-Frente-3) e o filtro `HIDDEN_CUSTOMER_TAGS` (`src/features/customers/utils/listFilters.ts` — oculta `pending_review`/`reviewed_not_customer` da tela Clientes) **não foram removidos** nesta entrega. Embora os 6 produtores originais de `customers pending_review` tenham sido fechados (Frentes A e C), o caminho `@lid` não resolvido acima continua produzindo uma população residual (pequena, mas contínua) de `customers` com essas mesmas tags — então o filtro de ocultação e as RPCs de conversão ainda têm alvo real e seguem necessários.
...
- **Deferimento registrado — `whatsapp-avatar-sync` não carimba leads**: o job (`supabase/functions/whatsapp-avatar-sync/`) só sabe escrever `customers.avatar_url`; nenhum produtor novo de lead (o `waha-webhook` da Frente A, nem os imports da Frente C) grava `leads.avatar_url` na criação — só o script B1 copia o avatar do customer no momento da migração do acervo. Na prática, um lead **novo** (criado depois do rollout desta frente, via inbound inédito ou import) nasce sem avatar e a conversa some com o ícone genérico até o job aprender a carimbar leads (fora do escopo declarado desta entrega — a spec §5 já previa esse deferimento "com registro", ver `docs/superpowers/specs/2026-07-18-funnel-frente3-waha-leads-design.md` linha 96). É uma regressão de UX leve e aceita, não um bug — segue como candidato a follow-up.
...
- **Convertendo um lead sem dono**: um lead `seller_id = null` (acervo/eco/import ainda não reaberto) pode ser convertido normalmente pelo `ConvertLeadModal` — o `customer` resultante nasce **sem carteira** (`seller_id` null) e `convertedBySellerId` (`ConvertLeadModal.tsx:102`, `currentUser?.sellerId ?? lead.sellerId`) também pode ficar `null` quando quem converte não tem `sellerId` próprio. Semântica aceita por construção: a carteira é atribuída depois, no fluxo normal de atribuição de cliente sem dono.
- **RLS oculta leads sem dono de não-staff**: a policy `leads_select` (`store_id = current_store_id() and (is_staff() or seller_id = current_seller_id())`) não tem exceção para `seller_id is null` — para um atendente (Vendedor/SDR/Externo), um lead ownerless retorna "não encontrado", então "Ver lead" a partir de uma conversa-lead falha silenciosamente para esse papel (o contato da conversa continua renderizando via RPC `SECURITY DEFINER`, que não passa pela RLS de linha). Registrado como candidato a follow-up junto com o gap de ficha lateral abaixo — não corrigido nesta task.

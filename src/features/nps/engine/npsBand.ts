 ...
 *
 * The target and the three cuts are store-configurable (Parâmetros tab, stored
 * in `nps_settings`). They arrive here as an *optional* argument defaulting to
 * the kit's values, so a surface that has no settings in hand still reads the
 * same as before and no call site is forced to thread configuration it does
 * not have.
 */
...
/** The kit's cuts — used by every surface until a store moves them. */
...
export const NPS_BAND_LABEL: Record<INpsBand, string> = {
  critical: "Crítica",
...
export function npsBandLabel(score: number, bands: INpsBandThresholds = DEFAULT_NPS_BANDS): string {
  return NPS_BAND_LABEL[npsBand(score, bands)];
}
 ...
 * ruler stops describing reality. The settings screen refuses to save an
 ...
 *
 * Derived from the same thresholds `npsBand` reads, so the ranges a settings
 * screen prints can never drift from the classification it is configuring.
 */

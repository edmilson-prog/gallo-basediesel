...
function MediaCell({ item }: { item: IConversationMediaItem }) {
  const isImage = item.kind === "image" || item.kind === "video";
  const { data: url } = useResolvedMediaUrl(isImage ? item.mediaUrl : undefined);
  ...
  return (
    <a
      ...
      className={cn("block text-left", !url && !isImage && "pointer-events-none")}
    >
      {isImage && url ? (
        <img
          src={url}
        ...
        <div className="flex h-[104px] flex-col justify-between rounded-sm bg-card p-2.5 ring-1 ring-inset ring-border">
          <Icon
            icon={
              item.kind === "audio"
                ? "mdi:microphone"

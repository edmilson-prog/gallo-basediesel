...
interface IProps {
  ...
  refetch: () => void;
  /**
   * Panel mode (Sheet): fixed filters + a scrollable body. Inline mode (fiche
   * tab): everything flows and the parent container owns the scroll.
   */
  scrollable?: boolean;
}

/**
 * Filterable gallery of message-derived media (image/audio/video/document).
 * Source-agnostic — fed by `useConversationMessageMedia` (one conversation) or
 * `useCustomerMessageMedia` (all of a customer's conversations).
 */
export function MessageMediaGallery({
  ...
  return (
    <div className={cn("flex flex-col", scrollable && "min-h-0 flex-1")}>
      ...
      {/* Body */}
      <div className={cn("mt-3 px-4 pb-4", scrollable && "min-h-0 flex-1 overflow-y-auto")}>
        {isLoading ? (

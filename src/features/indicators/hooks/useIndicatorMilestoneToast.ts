...
const STORAGE_KEY = "gallo-indicator-milestones";
...
/**
 * Hook that fires a toast when an active product indicator crosses a milestone
 * (50/80/100%). Each milestone per indicator is fired at most once per browser —
 * guarded via `localStorage` so reloads don't spam. The first invocation seeds
 * the storage with the current state so we don't re-fire for indicators already
 * past a milestone before the user opened the dashboard.
 *
 * Only indicators with `status === "ativo"` are considered.
 */
export function useIndicatorMilestoneToast(items: IIndicatorWithProgress[]): void {
  const seededRef = useRef(false);

  useEffect(() => {
    const activeItems = items.filter(({ indicator }) => indicator.status === "ativo");

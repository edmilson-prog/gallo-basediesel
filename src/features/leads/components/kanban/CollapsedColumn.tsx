 ...
 *
 * It forwards the droppable ref, so it stays a drop target: collapsing is a way
 * to save width, not to take the stage out of play, and a board where you
 * cannot drop into a folded column would force the user to unfold just to move
 * one card.
 */
export const CollapsedColumn = forwardRef<HTMLDivElement, ICollapsedColumnProps>(
  function CollapsedColumn({ stage, count, isDropTarget, onExpand }, ref) {
    return (
      <div
        ref={ref}
        className={cn(

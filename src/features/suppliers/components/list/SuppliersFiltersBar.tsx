...
/**
 * `category` is free text on the converged type (no fixed union — see
 * `utils/supplierDisplay.ts`), so this is the UI's own closed set of filter
 * chips, not a projection of a domain enum. It matches the four labels
 * `SUPPLIERS_STRINGS.categories` defines.
 */
const CATEGORIES = ["all", "parts", "services", "freight", "financial"] as const;

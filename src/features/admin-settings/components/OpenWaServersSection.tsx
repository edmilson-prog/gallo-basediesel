...
import { useWhatsAppOpenWaServersProvider } from "@/providers/data";
import type { IWhatsAppOpenWaServer } from "@/shared/types";
import { generateOpenWaServerKeyRef } from "../engine/openwaServerKeyRef";
import { setIntegrationSecret, deleteIntegrationSecret } from "../api/integrationSecrets";

/**
 * Owner-only CRUD of OpenWA servers (Integrações & Chaves). The global key is
 * written ONCE to the Vault here (via integration-secrets); the table stores
 ...
 */
export function OpenWaServersSection({ canEdit }: { canEdit: boolean }) {
  const provider = useWhatsAppOpenWaServersProvider();
  const [servers, setServers] = useState<IWhatsAppOpenWaServer[]>([]);
    ...
    <section className="rounded-lg border border-border bg-card">
      <header className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-semibold">Servidores OpenWA</h2>
      ...
      <div className="divide-y divide-border">
        ...
        {servers.map((s) => (
          <OpenWaServerRow key={s.id} server={s} canEdit={canEdit} onChanged={reload} />
        ))}
      </div>
    </section>
  );
...
function OpenWaServerForm({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const apiKeyRef = generateOpenWaServerKeyRef(n, existingRefs, suffix);
      await setIntegrationSecret(apiKeyRef, key, `Chave global OpenWA — ${n}`);
    ...
    <div className="space-y-3 px-4 py-4">
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="space-y-1.5">
          <Label htmlFor="openwa-server-name">Nome amigável</Label>
          <Input
            id="openwa-server-name"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="Ex.: AILA OpenWA Principal"
            disabled={!canEdit || saving}
          />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="openwa-server-url">Endpoint</Label>
          <Input
            id="openwa-server-url"
            className="font-mono"
            inputMode="url"
            value={baseUrl}
            onChange={(e) => setBaseUrl(e.target.value)}
            placeholder="https://openwa.seudominio.com"
            disabled={!canEdit || saving}
          />
        </div>
      </div>
      <div className="space-y-1.5">
        <Label htmlFor="openwa-server-key">Chave global da API</Label>
        <Input
          id="openwa-server-key"
          type="password"
...
function OpenWaServerRow({
  server,
  canEdit,
  onChanged,
}: {
  server: IWhatsAppOpenWaServer;
  canEdit: boolean;
  ...
  const handleRotate = async () => {
    ...
    try {
      await setIntegrationSecret(
        server.apiKeyRef,
        key,
        `Chave global OpenWA — ${server.name}`,
      );
